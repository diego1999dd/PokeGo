...
def list_pokemon():
    ...
    pokemon_list = []
    for i in range(1, 7): # Lista do ID 1 ao 6
        ...
        if poke_data:
            ...
            pokemon_list.append({
                ...
                "tipos": poke_data['tipos'], "favorito": status.Favorito if status else False,
...
def create_app():
    app = Flask(__name__)
    CORS(app, resources={r"/api/*": {"origins": "http://localhost:4200"}}, supports_credentials=True) # CORREÇÃO CORS: Permite que o Front-End acesse o Back-End
