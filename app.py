...
def fetch_pokemon_data(pokemon_id):
    # Função auxiliar para chamar a PokéAPI com resiliência e caching simples
    API_BASE = "https://pokeapi.co/api/v2/pokemon/"
    try:
        # Acesso direto à API pelo Back-End (Requisito 4)
        response = requests.get(f"{API_BASE}{pokemon_id}")
        ...
        print(f"Erro ao acessar PokéAPI: {e}")
        return None
...
def list_pokemon():
    ...
    # Itera sobre alguns IDs (isso deve ser otimizado na versão final)
    for i in range(1, 11): 
